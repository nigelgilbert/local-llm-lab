 ...
 *   "keep_drop_rule": "Keep \u2014 multi-opcode interaction surface is axis-critical for stateful_logic.",
...
describe(`mini-vm: bytecode interpreter (tier=${TIER_LABEL})`, () => {
  ...
  it('claw implements run handling all 13 opcodes + error cases', { timeout: TIMEOUT }, async () => {
    ...
    const targetExists = workspace.exists('vm.js');
    ...
    assert.equal(targetExists, true, 'vm.js must be created');
