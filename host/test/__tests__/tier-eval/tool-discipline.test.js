// Tool-call wrapping: percentage of streamed completions that land on
// stop_reason=tool_use for a one-tool prompt.
//
// All three tiers run through the same claw.gbnf grammar, so we expect
// near-1.0 across the board. A dip below the threshold signals that the
...
import { bridgeModel, TIER_LABEL, wrapRateThreshold } from '../../lib/tier.js';
...
describe(`tool-call wrapping (tier=${TIER_LABEL}, bridge=${bridgeModel})`, () => {
  it(
