...
export const BACKEND = process.env.BACKEND || 'llama-server';
...
export const bridgeModel = `claw-${SUFFIX}`;

// Used when invoking claw, which requires the `anthropic/` prefix.
export const clawModel = `anthropic/claw-${SUFFIX}`;
...
export const wrapRateThreshold = BACKEND === 'llama-server' ? 0.9 : 0.5;
