...
| 1.21 | Difficulty-extension test pack | **planned** — N=8 confirm hit the 26-test pack ceiling on both new tier configs (t16 98.3% / t32 99.5% done-only); Sprint 2's discrimination matrix needs cells where Wilson 95% CIs *can* separate. Author or import enough harder tests to drop done-only into the 50–85% band on at least one of t16/t32. **Hard gate** for Sprint 2 matrix publish — more reps on the saturated pack won't recover discrimination. Longest lead time of the open Sprint 1 follow-ups; kick off after 1.20 + 1.5 cuts so triage findings can inform what difficulty edges actually need coverage. Per the 2026-05-02 N=8 memo "Recommended next actions" item 3. |
