...
| 1.8 | Confirmatory dry-run on real claw call | **pending** — exercises 1.1 + 1.5 + 1.6 end-to-end against one real claw invocation per tier. Requires the host-side llama-server plist swap; user-coordinated, not autonomous. Recommended next step before kicking off the full overnight: `EVAL_REPS=1 EVAL_TIERS=64 host/test/scripts/run-overnight-screen.sh` while `thermal-watch.sh` runs in a separate terminal — produces ~35 registry rows for tier-64 and proves the wiring before committing an overnight budget. |
