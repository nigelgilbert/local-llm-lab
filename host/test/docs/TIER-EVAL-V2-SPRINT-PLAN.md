...
**Date:** 2026-04-29
**Status:** Sprint 0 closed. Sprint 1 closed through 1.20 (2026-05-02; N=8 t16 64k confirmatory landed 177/195 = 90.8% pass-rate, 99.4% done-only, 6.25% typed harness_error). Sprint 1.21 (difficulty-extension test pack) is the only remaining open follow-up before Sprint 2.
...
| 1.19 | Tier-32 param-tuning + model swap; tier-16 model swap | **done** 2026-05-02 — model swap landed on both tiers (Qwen3.5-9B unified base) after sweeps 1–4. **Lock-ins:** t16 = `qwen35-9b-iq4xs-ctx32k-v6antiloop-pp01` (cell E), t32 = `qwen35-9b-q5kxl-ctx64k-v7noreppen-pp01` (cell B2). Old baselines `qwen25-7b-instruct-q5km-ctx32k-v1prod-pp01` / `qwen3-14b-q4km-ctx32k-v1prod-pp01` flipped to archived in `lib/model_configs.json`. **Confirm N=8 (208 attempts each, 4×N=4 chunks per tier):** t16 84.6% pass-all / 98.3% done-only; t32 88.9% pass-all / 99.5% done-only. vs 1.18 baseline: t16 +47.4 pp pass-all / +52.0 pp done-only; t32 +57.7 pp / +66.0 pp. **1.18 t16↔t32 Pareto inversion is closed** — t32 now cleanly dominates t16 on both metrics; manifesto framing of t32 as "instruction-following tier that doesn't dominate" must be revised before Sprint 2 ships. Failure modes shifted: t16 bottlenecked by harness-error rate (13.5%), t32 by 64k-context timeouts (7.7%). Both tiers now hit the 26-test pack ceiling — discrimination matrix needs a harder pack before Sprint 2 (see TIER-EVAL-MEMO-20260502-n8-confirm-vs-baseline.md). Source rows: `host/test/.claw-runtime/run_registry.t{16,32}-confirm-n4-chunk{1,2}-*.csv`. |
| 1.20 | t16 harness-error triage | **done** 2026-05-02 — root cause: 28/208 = 13.5% errors were 100% llama-server context-overflow rejections at t16's 32k ceiling. Confirmed 4/4 in N=1 t16 32k diagnostic ([t16-1.20-diag-n1-iq4xs-32k-...](../.claw-runtime/run_registry.t16-1.20-diag-n1-iq4xs-32k-20260502-0346.csv)); cross-validated by 5/6 t32 errors clustering at the 64k ceiling. Three changes shipped: (a) `iter_distribution_logger.py` captures `failure_class`+`failure_message_tail` on LiteLLM failures; (b) t16 lock-in moved to `qwen35-9b-iq4xs-ctx64k-v6antiloop-pp01` (same model+sampler, n_ctx 32768→65536); (c) `run_row.js` relabels kill-the-run typed context-overflow as `terminal_status='harness_error', harness_error='context_overflow', passed=null` (gated on claw exit ≠ 0; matches three typed patterns: `BadRequestError` + "exceeds the available context size", and `InternalServerError`/`APIError` + "Context size has been exceeded" — both surface the same root cause from different LiteLLM paths). **N=8 t16 64k confirmatory ([t16-1.20-confirm-n8-iq4xs-64k-...](../.claw-runtime/run_registry.t16-1.20-confirm-n8-iq4xs-64k-20260502-0459.jsonl), 208 attempts): 177/195 = 90.8% pass-rate (harness_error excluded; +6.2 pp vs 1.19 32k baseline), 177/178 = 99.4% done-only, 13/208 = 6.25% typed harness_error (-7.25 pp), 17/208 = 8.2% timeouts (+7.7 pp).** Failures reshape from typed overflows (28→13) to wallclock timeouts (1→17): expression-eval is the dominant residual overflow site (7/8 reps; overshoots up to 76 189 tokens vs 65 536 ceiling); csv-parser is the new dominant timeout test (8/8 reps at 240s). Detector widening was load-bearing — 7 of 13 typed overflows surfaced as `InternalServerError`/`APIError` (not `BadRequestError`), so without the multi-pattern detector those rows would have polluted the `error` column instead of dropping out of the Layer-A denominator. Partial fix confirmed: 64k cuts harness_error by ~half but trades against an 8.2% timeout rate; both must be reported separately in Sprint 2's matrix. Pre-1.20 1.19 confirm rows are not retroactively relabeled (failure_class absent in pre-1a slices). See [TIER-EVAL-MEMO-20260502-n8-confirm-vs-baseline.md](TIER-EVAL-MEMO-20260502-n8-confirm-vs-baseline.md) "Sprint 1.20 closure". |
| 1.21 | Difficulty-extension test pack | **planned** — N=8 confirm hit the 26-test pack ceiling on both new tier configs (1.20 N=8 t16 64k post-fix done-only 99.4%; 1.19 t32 done-only 99.5%); Sprint 2's discrimination matrix needs cells where Wilson 95% CIs *can* separate. Author or import enough harder tests to drop done-only into the 50–85% band on at least one of t16/t32. **Hard gate** for Sprint 2 matrix publish — more reps on the saturated pack won't recover discrimination. Longest lead time of the remaining Sprint 1 follow-ups. Per the 2026-05-02 N=8 memo "Recommended next actions" item 3 and the 1.20 closure note "Sprint 2 implications". |
