...
| 1.9 | Multi-tier confirmatory before full overnight | **pending** — recommended scope: `EVAL_REPS=1 EVAL_TIERS="16 32 64" host/test/scripts/run-overnight-screen.sh` with `thermal-watch.sh` running. ~30–60 min wallclock, exercises plist swaps + per-tier model_config_id resolution + ~105 registry rows. Final gate before committing an overnight budget. |
