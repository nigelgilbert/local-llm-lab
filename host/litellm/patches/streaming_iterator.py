...
#
# ============================================================================
# CYBERIA LOCAL-LLM-LAB PATCH — see `PATCH (cyberia local-llm-lab)` markers
# below for the actual edits. This header explains scope and maintenance.
#
# Bug fixed:
#   When an OpenAI-shaped upstream chunk opens a NEW tool_use content block
#   and the same trigger chunk also carries tool_call arguments (any
#   `partial_json` payload), the upstream iterator emits content_block_stop
#   + content_block_start and DROPS the trigger chunk's args. Anthropic
#   `content_block_start` for tool_use has empty `input: {}` per spec — the
#   args MUST come via subsequent `content_block_delta(input_json_delta)`
#   events. Without those, clients see tool_use blocks with empty input.
#
# Affected upstreams (i.e. when this patch matters):
#   - ANY Ollama model with tool-use. Ollama parses `<tool_call>...</tool_call>`
#     atomically and always emits args in a single chunk → args ALWAYS lost
#     without this patch. Holds for Qwen3-Coder, Qwen2.5-Coder, llama3.2,
#     mistral-with-tools, etc. The patch is model-agnostic at the Ollama
#     layer; switching Ollama models doesn't require patch changes.
#   - Real OpenAI / OpenAI-compat with streaming tool calls. The first args
#     chunk (typically `{"` plus a few chars) is dropped, corrupting the JSON
#     args string. Less obvious failure than the Ollama case but still wrong.
#
# NOT affected:
#   - Anthropic-native passthrough (different code path entirely).
#   - Plain text completions (no tool_use blocks → patch never triggers).
#
# Safety / blast radius:
#   The patch only adds an extra emission when ALL of: should_start_new_block,
#   processed_chunk is a content_block_delta, delta type is input_json_delta,
#   and partial_json is truthy. Text-only flows and tool-flows-without-args
#   are untouched. Cannot break non-tool-using requests.
#
# Maintenance:
...
#   the marked sites:
#     docker exec cyberia-litellm cat /usr/lib/python3.13/site-packages/\
...
#   If upstream fixes the bug (check the Anthropic adapter in litellm), this
#   whole patch file becomes redundant — delete it and the volume mount.
# ============================================================================
import json
import traceback
        ...
        try:
            ...
            for chunk in self.completion_stream:
                ...
                if should_start_new_block and not self.sent_content_block_finish:
                    ...
                    #
                    # PATCH (cyberia local-llm-lab): for tool_use blocks, the
                    ...
                    # assumption that content_block_start carries the data,
                    # but content_block_start for tool_use has empty input: {}
                    # — args must come via input_json_delta or clients see no
                    # arguments. Re-emit processed_chunk in that case.
                    self.chunk_queue.append(
                        {
                    ...
                    if (
                        ...
                        and processed_chunk.get("delta", {}).get("type")
                        == "input_json_delta"
        ...
        try:
            ...
            async for chunk in self.completion_stream:
                ...
                if not self.queued_usage_chunk:
                    if should_start_new_block and not self.sent_content_block_finish:
                        # PATCH (cyberia local-llm-lab): same bug as the sync
                        ...
                        # entire tool_call in one chunk and dropping the args
                        # leaves clients with empty input: {}.
                        ...
                        # 3. PATCH: re-emit args if the trigger chunk carried them
                        if (
                            isinstance(processed_chunk, dict)
                            and processed_chunk.get("type") == "content_block_delta"
                            and processed_chunk.get("delta", {}).get("type")
