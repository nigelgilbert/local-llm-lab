...
```sh
docker exec mac-llm-lab-litellm cat /usr/lib/python3.13/site-packages/litellm/llms/anthropic/experimental_pass_through/adapters/streaming_iterator.py > /tmp/upstream.py
diff host/litellm/patches/streaming_iterator.py /tmp/upstream.py
# re-apply the PATCH (mac-llm-lab mac-llm-lab) blocks into a fresh copy, mv into place, docker compose up -d --force-recreate.
```
If upstream fixes the bug, delete the patch file and the volume mount.

**Quick verification:**
```sh
docker exec mac-llm-lab-litellm grep -c "PATCH (mac-llm-lab mac-llm-lab)" \
...
curl -fsS http://mac-llm-lab.local:4000/health/liveliness
...
- `mac-llm-lab warm -p <profile>` from [`../../client/`](../../client/) preheats whichever Ollama model the bridge will route to.
