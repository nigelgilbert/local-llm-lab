# Mac LLM Lab — Architecture & Build Plan
...
- exposes **Open WebUI** on the LAN over **plain HTTP at port 80** via mDNS hostname `mac-llm-lab.local`
...
- ships a thin client-side shell CLI named `mac-llm-lab` that orchestrates from laptops
...
- Mac's `LocalHostName` set to `mac-llm-lab` → `http://mac-llm-lab.local` resolves on any LAN client
...
mac-llm-lab chat -p general                      # daily driver preselected
mac-llm-lab chat -p fast                         # snappy triage preselected
mac-llm-lab chat -p reasoning                    # math / planning preselected
mac-llm-lab chat -p digest                       # long-context extract preselected
mac-llm-lab chat -p analyze                      # long-context reasoning preselected
mac-llm-lab chat -p digest -q "summarise..."     # preselect + prefilled query
mac-llm-lab status                               # SSH host → mac-llm-lab-hostctl status
mac-llm-lab warm -p reasoning                    # preload model
```
...
| Feature | Phase |
|---|---|
| `mac-llm-lab wake` (Wake-on-LAN), `mac-llm-lab cli` TTY tunnel | 2 |
...
Open WebUI binds directly to host port 80 on the LAN interface. mDNS gives us `mac-llm-lab.local` for free; no reverse proxy needed for hostname stability. LAN-only HTTP is acceptable security given the network passport gating. Caddy retrofits cleanly later — adding it does not break the URL contract.
...
`mac-llm-lab` on client machines is a small POSIX shell helper. Heavy logic lives on the host as `mac-llm-lab-hostctl`. Compose paths, model names, health checks all evolve on the host without forcing client updates.

### 1.6 mDNS is the LAN hostname strategy (validated)

Mac `LocalHostName=mac-llm-lab`; `mac-llm-lab.local` resolves cross-device on this LAN (verified 2026-04-25 — Bonjour discovery works through the AT&T gateway on the main SSID). **Fallback:** stable LAN IP via router DHCP reservation by MAC, in case mDNS fails on a guest device or guest SSID.
...
|-------------------|          |-------------------------------------------|
| mac-llm-lab CLI       | --SSH--> | Remote Login (SSH)                        |
...
Phase 2+ retrofits: mac-llm-lab wake (WoL), tailscale serve (remote HTTPS),
...
- LAN access: `http://mac-llm-lab.local` (port 80)
- Mac hostname set via `sudo scutil --set LocalHostName mac-llm-lab`
...
- **`mac-llm-lab` shell CLI.** Reads `~/.config/mac-llm-lab/config.env`. SSHes host for control actions (`mac-llm-lab-hostctl up | status | warm`). Opens browser to deep-link URL. Phase 2+: WoL packet, TTY chat tunnel.
...
- **Power-user mode** — you. `mac-llm-lab` CLI from your laptop.
- **Guest mode** — LAN guests. Browser only, bookmark `http://mac-llm-lab.local`.

### 7.3 Open WebUI account policy

- First account = admin (you) — first user to sign up at `mac-llm-lab.local` automatically becomes admin
...
`mac-llm-lab chat -p X` flow (no wake):
1. SSH host → `mac-llm-lab-hostctl up` (idempotent — verifies Open WebUI + Ollama healthy)
2. SSH host → `mac-llm-lab-hostctl warm <profile>` (preloads via Ollama API with `keep_alive=30m`)
3. Open browser → `http://mac-llm-lab.local/?model=<profile>`

### 8.2 `mac-llm-lab chat` URL contract

```text
http://mac-llm-lab.local/                                       # bare bookmark
http://mac-llm-lab.local/?model=general                         # profile preselect
http://mac-llm-lab.local/?model=digest&q=Summarise...           # profile + prefilled query
```
...
`mac-llm-lab-hostctl warm <profile>` calls Ollama's `/api/generate` with empty prompt + `keep_alive=30m`. Verify with `ollama ps` / `/api/ps`.

### 8.4 Phase 2 — `mac-llm-lab wake`
...
`mac-llm-lab` (client) calls `mac-llm-lab-hostctl` (host) over SSH.

```sh
mac-llm-lab-hostctl up              # docker compose up -d, verify Open WebUI + Ollama
mac-llm-lab-hostctl down            # docker compose down
mac-llm-lab-hostctl status          # health (services + ollama ps)
mac-llm-lab-hostctl warm <profile>  # preload mapped model via Ollama API
mac-llm-lab-hostctl openui-url      # print canonical browser URL
```
...
mac-llm-lab/
  ...
  client/
    mac-llm-lab                        # single-file POSIX shell client
    config.env.example
    README.md
...
- **Phase 2 — Wake + Remote:** WoL flow (`mac-llm-lab wake`); Tailscale + `tailscale serve` for remote HTTPS; Caddy retrofit if needed; `mac-llm-lab cli` TTY tunnel.
...
If guest SSID isolation breaks `mac-llm-lab.local` resolution, look for "mDNS reflector" / "Bonjour gateway" setting on the AT&T residential gateway.
...
1. **Mac prep**
   ...
   - `LocalHostName=mac-llm-lab` (done 2026-04-25)
...
3. **Docker stack**
   ...
   - `docker compose up -d`; verify `curl http://mac-llm-lab.local`
...
5. **Host control script**
   - `host/scripts/mac-llm-lab-hostctl` (POSIX sh): up, down, status, warm, openui-url
   - Idempotent up; status returns clean text/JSON

6. **Client CLI**
   - `client/mac-llm-lab` (POSIX sh): chat, status, warm
   - `client/lib/{config,ssh,openui,profiles}.sh`
   - `~/.config/mac-llm-lab/config.env` schema documented

7. **End-to-end acceptance**
   - `mac-llm-lab chat -p general` → browser opens with `general` preselected; conversation works
   - `mac-llm-lab chat -p digest -q "..."` → preselects + prefills query
   - LAN guest device → `http://mac-llm-lab.local` → signs up → immediately chats with all five profiles
   ...
   - Restart: `mac-llm-lab-hostctl down && up` → state preserved (named volume)
...
- [ ] DHCP reservation at router (parallel task; Ethernet + Wi-Fi MACs both reserved to `mac-llm-lab`)
